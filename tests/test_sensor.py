...
@pytest.mark.parametrize(
    ...
    [
        ...
        (4, "Alto", "mdi:biohazard"),     # Level 4 currently maps to Alto (based on func logic)
        (99, "Alto", "mdi:help-circle-outline"), # Level 99 currently maps to Alto (based on func logic)
    ],
)
def test_get_pollen_level_details(level, expected_name, expected_icon):
    """Test the pollen level details helper function."""
    # Provide dummy threshold values for the test as the function requires them.
    medium_threshold = 2
    high_threshold = 3
    level_name, _, icon = get_pollen_level_details(level, medium_threshold, high_threshold)
...
async def test_sensor_setup_and_state(hass: HomeAssistant, mock_requests_post) -> None:
    ...
    # Check sensor state for a specific pollen type (entities should have been added)
    ...
    # Use the helper to get expected level string (or derive from value/thresholds)
    medium_thr = expected_record.get('med', 2) # Use defaults if not in mock
    high_thr = expected_record.get('alt', 3)
    level_name, _, _ = get_pollen_level_details(expected_record['nivel'], medium_thr, high_thr)
    assert state.attributes.get("pollen_level") == level_name
    assert state.attributes.get("station_id") == "28079016"
    assert state.attributes.get("pollen_type") == "Platanus" # Check against 'nombre' from mock
...
async def test_sensor_update_failed(hass: HomeAssistant, mock_requests_post) -> None:
    ...
    # The coordinator's _async_update_data should catch the Timeout and raise UpdateFailed
...
@pytest.mark.usefixtures("mock_requests_post") # Apply fixture to this test too
